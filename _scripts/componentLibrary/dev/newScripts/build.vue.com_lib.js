...
async function build() {
  ...
  await Promise.all([
    new Promise((resolve, reject) => {
      webpack(getWebpckConfig({ entry: { edit: editCodePath, ...componentsEntry}, outputPath: compileProductFolderPath, externals: [externalsMap], postCssOptions: getPostCssOption({}) }, webpackMergeConfig), (err, stats) => {
    ...
    new Promise((resolve, reject) => {
      webpack(getWebpckConfig({ entry: { 'rt': rtCodePath }, outputPath: compileProductFolderPath, externals: [externalsMap], postCssOptions: getPostCssOption(mybricksJson) }, webpackMergeConfig), (err, stats) => {
        if (err || stats.hasErrors()) {
          console.error(err || stats.compilation.errors);
          reject(err || stats);
        }
